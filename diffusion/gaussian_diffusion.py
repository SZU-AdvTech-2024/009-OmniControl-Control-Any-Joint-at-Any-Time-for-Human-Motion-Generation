...
class GaussianDiffusion:
    ...
    def calc_grad_scale(self, mask_hint):
        """
        Calculate the scale of the gradient
        """
    ...
    def text_to_hint(self, detailed_text):
    # Example implementation: Parse detailed_text and generate joint coordinates
        hint = []
        for text in detailed_text:
            joint_coords = self.parse_text_to_coordinates(text)  # Custom parser
            hint.append(joint_coords)
        return torch.stack(hint, dim=0)  # Return as tensor
    
    def parse_text_to_coordinates(self, text):
        # Example: Extract joint coordinates from text
        # Input: "Left hand moves to (0.5, 1.2, 0.3)"
        # Output: torch.tensor([0.5, 1.2, 0.3])
        coords = re.findall(r"\(([-\d.]+), ([-\d.]+), ([-\d.]+)\)", text)
        if coords:
            return torch.tensor([float(c) for c in coords[0]])
        else:
            return torch.zeros(3)  # Default to zero if no coordinates found
    ...
    def guide(self, x, t, model_kwargs=None, t_stopgrad=-10, scale=.5, n_guide_steps=10, train=False, min_variance=0.01):
        ...
        # process hint
        # print("model_kwargs:",model_kwargs['y']['detailed_text'])
        hint = model_kwargs['y']['detailed_text']
        # print("detailed_text:", detailed_text)
        # sys.exit()
