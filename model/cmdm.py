...
class CMDM(torch.nn.Module):
    ...
    def cmdm_forward(self, x, timesteps, y=None, weight=1.0):
        ...
        x对应diffusion中每个timestep下的运动信息
        y是数据集经过collate后的字典，里面包含text、hint、uncond等keys，细粒度文本代替control signal就需要使用detailed text代替hint控制
        """
        # TODO 3.0:修改原先的cmdm，将字典y调用hint的部分替换成细粒度文本detailed_text，然后用text encoder进行编码
        # 注意：原先cmdm中的mask操作和spatial encoder不再需要，对应seq_mask变量以及input_hint_block函数
        emb = self.embed_timestep(timesteps)  # [1, bs, d]

        # force_mask = y.get('uncond', False)
        print("detailed_text:", y['detailed_text'])
        guided_hint = self.encode_text(y['detailed_text'])
        # print("detailed_text:",enc_text)

        guided_hint = self.embed_text(guided_hint)
...
# 时间步嵌入 (TimestepEmbedder)：将扩散模型的时间步 timesteps 转化为嵌入向量。这些嵌入向量用于在每个时间步对输入数据进行引导。
